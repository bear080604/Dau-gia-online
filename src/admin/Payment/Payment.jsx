...
function Payment() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const data = await response.json();
      // Fix: Handle potential wrapped data structure (e.g., {data: [...]}) consistently
      ...
      const transformedPayments = paymentsData.map(payment => {
        ...
        return {
          ...
          statusValue: payment.status, // Add for robust editing
  ...
  const openPaymentModal = (mode, payment = null) => {
    ...
    if (payment) {
      setPaymentForm({
        ...
        status: payment.statusValue // Use the API key directly for robustness
      });
  ...
  // Get unique contract IDs for filter dropdown (nếu cần, nhưng giờ dùng contracts state)
            ...
            <div className={styles.modalBody}>
              <div>
                ...
                >
                  ...
                  {/* ✅ Giờ map contracts sẽ work vì đã fix fetch */}
                  {contracts.map(c => (
                    <option key={c.contract_id} value={`HD${c.contract_id}`}>
                      #{c.contract_id} - {getStatusLabel(c.status)} - {Number(c.final_price).toLocaleString('vi-VN')}₫
                    </option>
                  ))}
                </select>
              </div>
