  ...
  function Register() {
    ...
    const [formData, setFormData] = useState({
      ...
      online_contact_method: accountType === "auctioneer" ? "" : undefined,
      certificate_number: accountType === "auctioneer" ? "" : undefined,
      certificate_issue_date: accountType === "auctioneer" ? "" : undefined,
      certificate_issued_by: accountType === "auctioneer" ? "" : undefined,
    ...
    const [auctioneerCardFront, setAuctioneerCardFront] = useState(accountType === "auctioneer" ? null : undefined);
    const [auctioneerCardBack, setAuctioneerCardBack] = useState(accountType === "auctioneer" ? null : undefined);
    ...
    const validateForm = () => {
      ...
      if (!formData.full_name.trim()) {
        newErrors.full_name = accountType === "personal" 
      ...
      // if (!formData.terms_accepted) newErrors.terms_accepted = "Vui lòng chấp nhận điều khoản.";
  ...
  data.append("terms_accepted", formData.terms_accepted); // Đảm bảo gửi terms_accepted

  console.log("Dữ liệu gửi đi:", Object.fromEntries(data)); // Log dữ liệu
  ...
  try {
    ...
    console.error("Lỗi API:", err.response ? err.response.data : err.message); // Log lỗi chi tiết
    ...
    return (
      <div className={styles.registerContainer}>
        ...
        <div className={styles.accountTypeButtons}>
          <button
            type="button"
            className={`${styles.accountTypeButton} ${accountType === "personal" ? styles.active : ""}`}
          ...
          <button
            type="button"
            className={`${styles.accountTypeButton} ${accountType === "auctioneer" ? styles.active : ""}`}
            onClick={() => setAccountType("auctioneer")}
            disabled={isLoading}
          >
          Đấu giá viên
          </button>
        ...
        <form onSubmit={handleSubmit} className={styles.registerForm} encType="multipart/form-data">
          <div className={styles.formGroup}>
            <label>{accountType === "personal" ? "Họ và tên" : accountType === "auctioneer" ? "Họ và tên đấu giá viên" : "Họ và tên người đại diện pháp luật"} <span className={styles.required}>*</span></label>
            <input
              type="text"
              name="full_name"
          ...
          {/* <div className={styles.formGroup}>
            <label>
              <input
                ...
                disabled={isLoading}
              />{" "}
              Tôi đồng ý với <a href="https://example.com/terms" target="_blank">điều khoản sử dụng</a>
            </label>
            {(clientErrors.terms_accepted || errors.terms_accepted) && (
              <p className={styles.errorMsg}>{clientErrors.terms_accepted || errors.terms_accepted[0]}</p>
            )}
          </div> */}
